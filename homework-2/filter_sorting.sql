...
SELECT DISTINCT ship_city, ship_country FROM public.orders
ORDER BY ship_country, ship_city # Но в скриншоте отсутствует сортировка
...
SELECT AVG(shipped_date - order_date) FROM public.orders
...
SELECT MIN(unit_price), MAX(unit_price) FROM public.products
